...
class LinearTrajectory(Trajectory):
    def __init__(self, start_position, goal_position, total_time, orientation=np.array([0, 1, 0, 0])):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            pos = self.start_position + self.distance / 2 + self.v_max * (time - .5*self.total_time) - .5 * self.acceleration * (time - .5*self.total_time)**2
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            linear_vel = self.v_max - self.acceleration *(time - .5 * self.total_time)
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            theta = np.pi + self.angular_v_max * (time - .5*self.total_time) - .5 * self.angular_acceleration * (time - .5*self.total_time)**2
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            theta = np.pi + self.angular_v_max * (time - .5*self.total_time) - .5 * self.angular_acceleration * (time - .5*self.total_time)**2
            theta_dot = self.angular_v_max - self.angular_acceleration * (time - .5*self.total_time)

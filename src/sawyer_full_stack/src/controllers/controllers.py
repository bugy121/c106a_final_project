...
class PIDJointVelocityController(Controller):
    ...
    def step_control(self, target_position, target_velocity, target_acceleration):
        ...
        p_control = self.Kp @ (target_position - current_position)
        i_control = self.Ki @ self.integ_error
        d_control = self.Kd @ (target_velocity - current_velocity)

        controller_velocity = target_velocity + p_control + i_control + d_control
